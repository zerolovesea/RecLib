...
def example_movielens_100k_dssm():
    ...
    # user sparse
    user_sparse_features = [SparseFeature(col, vocab_size=int(df[col].max()) + 1, embedding_dim=4) for col in user_sparse_cols if col != 'user_id']
    user_sparse_features.append(SparseFeature('user_id', vocab_size=int(df['user_id'].max()) + 1, embedding_dim=32))

    # item sparse
    item_sparse_features = [SparseFeature(col, vocab_size=int(df[col].max()) + 1, embedding_dim=4) for col in item_sparse_cols if col != 'item_id']
    # item_id
