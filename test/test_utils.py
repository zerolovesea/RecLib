...
def assert_model_output_range(output: torch.Tensor, min_val: float = 0.0, max_val: float = 1.0):
    ...
    logger.info(f"Output range assertion passed: [{output.min().item():.4f}, {output.max().item():.4f}]")
...
def run_model_forward_backward(model: torch.nn.Module, 
                                 data: Dict[str, torch.Tensor],
                                 targets: torch.Tensor,
                                 loss_fn: torch.nn.Module) -> Dict[str, Any]:
    """
...
def run_model_inference(model: torch.nn.Module,
