...
class TestDataProcessorNumericFeatures:
    ...
    @pytest.mark.parametrize(
        "scaler_type", ["standard", "minmax", "robust", "maxabs", "log", "none"]
    )
    ...
    def test_numeric_fill_na_custom(self):
        ...
        data = pd.DataFrame({"feature": [1.0, np.nan, 3.0, np.nan, 5.0]})
...
class TestDataProcessorSparseFeatures:
    ...
    def test_sparse_feature_missing_values(self):
        ...
        data = pd.DataFrame({"user_id": ["user_1", None, "user_3", None, "user_5"]})
...
class TestDataProcessorSequenceFeatures:
    ...
    def test_sequence_truncation(self, truncate):
        ...
        data = pd.DataFrame({"seq": ["a,b,c,d,e,f,g,h,i,j"]})  # 10 items

        processor = DataProcessor()
        processor.add_sequence_feature(
            "seq", encode_method="label", max_len=5, truncate=truncate, separator=","
        )
    ...
    def test_sequence_padding(self):
        ...
        data = pd.DataFrame({"seq": ["a,b", "c", "d,e,f"]})

        processor = DataProcessor()
        processor.add_sequence_feature(
            "seq", encode_method="label", max_len=5, pad_value=0, separator=","
        )
...
class TestDataProcessorTargetFeatures:
    ...
    def test_add_binary_target(self):
        ...
        data = pd.DataFrame({"label": [0, 1, 1, 0, 1]})
...
class TestDataProcessorSaveLoad:
    ...
    def test_save_and_load(self, tmp_path):
        ...
        processor.add_sparse_feature("user_id", encode_method="label")
        processor.add_sequence_feature(
            "history", encode_method="label", max_len=5, separator=","
        )
        processor.add_target("label", target_type="binary")
        ...
        for key in original_result.keys():
            ...
            np.testing.assert_array_almost_equal(
                original_result[key], loaded_result[key], decimal=5
            )
...
class TestDataProcessorIntegration:
    ...
    def test_mixed_features_pipeline(self):
        ...
        processor.add_sequence_feature(
            "item_history", encode_method="label", max_len=10, separator=","
        )
        processor.add_target("label", target_type="binary")
    ...
    def test_transform_with_return_dataframe(self):
        ...
        data = pd.DataFrame(
            {"age": [25, 30, 35], "user_id": ["u1", "u2", "u3"], "label": [0, 1, 1]}
        )
    ...
    def test_processor_summary(self):
        ...
        processor.add_sparse_feature("user_id", encode_method="label")
        processor.add_sequence_feature(
            "history", encode_method="hash", hash_size=5000, max_len=20
        )
        processor.add_target("label", target_type="binary")
